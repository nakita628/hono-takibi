...
describe('README — Custom Validation', () => {
  it('x-refine: short password fires the first refine message', async () => {
    ...
    expect(body.errors?.some((e) => e.detail === 'Password must be at least 8 characters')).toBe(
      true,
    )
  })
  ...
  it('x-refine: missing uppercase fires the second refine message', async () => {
    ...
    expect(body.errors?.some((e) => e.detail === 'Password must contain an uppercase letter')).toBe(
      true,
    )
  })
...
describe('Numeric — thick coverage', () => {
  ...
  it('required vs type: distinct messages', async () => {
    ...
    expect(missingBody.errors).toStrictEqual([
    ...
    expect(wrongBody.errors).toStrictEqual([
...
describe('Array — thick coverage', () => {
  ...
  it('maxItems: 6-item array rejected', async () => {
    ...
    expect(body.errors).toStrictEqual([
      { pointer: '/tags', detail: 'tags must have at most 5 items' },
    ])
  })
  ...
  it('uniqueItems: duplicates rejected', async () => {
    ...
    expect(body.errors).toStrictEqual([
      { pointer: '/unique/1', detail: 'unique must contain distinct integers' },
    ])
  })
  ...
  it('minContains: 0 premiums rejected', async () => {
    ...
    expect(body.errors?.some((e) => e.detail === 'basket must contain at least 1 premium')).toBe(
      true,
    )
  })
  ...
  it('maxContains: 4 premiums rejected', async () => {
    ...
    expect(body.errors?.some((e) => e.detail === 'basket must contain at most 3 premium')).toBe(
      true,
    )
  })
...
describe('Object — thick coverage', () => {
  ...
  it('patternProperties: x_ key with non-string rejected', async () => {
    ...
    expect(body.errors).toStrictEqual([
      { pointer: '/namespace/x_one', detail: 'x_ keys must be strings' },
    ])
  })
  ...
  it('dependentRequired: cc without billing rejected', async () => {
    ...
    expect(body.errors).toStrictEqual([
      { pointer: '/dependent/billing', detail: 'billing required when cc provided' },
    ])
  })
