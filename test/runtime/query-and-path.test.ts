...
import app from '../hosts/query-and-path-app'
...
describe('Query parameter coercion', () => {
  it('coerces ?limit=10&price=3.14&active=true into number/number/boolean', async () => {
    const res = await app.request('/search?q=hello&limit=10&price=3.14&active=true')
  ...
  it('coerces active=false to boolean false', async () => {
    const res = await app.request('/search?q=x&limit=1&price=0&active=false')
    expect(res.status).toBe(200)
    const body = (await res.json()) as { active: boolean; activeType: string }
    expect(body.active).toBe(false)
    expect(body.activeType).toBe('boolean')
  })

  it('rejects non-numeric limit with 422', async () => {
    const res = await app.request('/search?q=x&limit=abc&price=0&active=true')
    expect(res.status).toBe(422)
  })

  it('rejects non-boolean active with 422', async () => {
    const res = await app.request('/search?q=x&limit=1&price=0&active=maybe')
    expect(res.status).toBe(422)
  })

  it('applies defaults when optional query is omitted', async () => {
    const res = await app.request('/search/optional')
    expect(res.status).toBe(200)
    const body = await res.json()
    expect(body).toStrictEqual({
      limit: 10,
  ...
  it('coerces explode-array ?ids=1&ids=2 into number[]', async () => {
    const res = await app.request('/search/array?ids=1&ids=2&ids=3&flags=true&flags=false')
    expect(res.status).toBe(200)
    const body = await res.json()
    expect(body).toStrictEqual({
      ids: [1, 2, 3],
...
describe('Path parameter coercion', () => {
  it('coerces /items/integer/42 into number', async () => {
    const res = await app.request('/items/integer/42')
    expect(res.status).toBe(200)
    const body = await res.json()
  ...
  it('coerces /items/int32/2147483647 into number', async () => {
    const res = await app.request('/items/int32/2147483647')
    expect(res.status).toBe(200)
    const body = await res.json()
  ...
  it('coerces /items/int64/9007199254740993 into bigint', async () => {
    const res = await app.request('/items/int64/9007199254740993')
    expect(res.status).toBe(200)
    const body = await res.json()
  ...
  it('coerces /items/number/3.14 into number', async () => {
    const res = await app.request('/items/number/3.14')
    expect(res.status).toBe(200)
    const body = await res.json()
  ...
  it('coerces /items/boolean/true into boolean', async () => {
    const res = await app.request('/items/boolean/true')
    expect(res.status).toBe(200)
    const body = await res.json()
  ...
  it('coerces /items/boolean/false into boolean', async () => {
    const res = await app.request('/items/boolean/false')
    expect(res.status).toBe(200)
    const body = await res.json()
  ...
  it('coerces multiple path segments /items/mixed/7/sub/true', async () => {
    const res = await app.request('/items/mixed/7/sub/true')
    expect(res.status).toBe(200)
    const body = await res.json()
  ...
  it('rejects /items/integer/abc with 422', async () => {
    const res = await app.request('/items/integer/abc')
    expect(res.status).toBe(422)
  })

  it('rejects /items/boolean/maybe with 422', async () => {
    const res = await app.request('/items/boolean/maybe')
    expect(res.status).toBe(422)
  })
})
