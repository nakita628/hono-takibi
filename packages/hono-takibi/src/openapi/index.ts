...
> {
  try {
    if (typeof input === 'string' && input.endsWith('.tsp')) {
      ...
      const spec = (await SwaggerParser.validate(tsp.value as unknown as string)) as OpenAPI
    ...
    const spec = (await SwaggerParser.validate(input)) as OpenAPI
    return { ok: true, value: spec }
  } catch (e) {
 ...
 */
type BaseOpenAPI = Awaited<ReturnType<typeof SwaggerParser.validate>>
