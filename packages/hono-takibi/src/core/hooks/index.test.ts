...
`)
        ...
        expect(result).toStrictEqual(
          `Generated swr hooks written to ${path.join(dir, 'swr')}/*.ts (index.ts included)`,
        )
      } finally {
        fs.rmSync(dir, { recursive: true, force: true })
      }
...
`)
  ...
  describe('swr (no args operations)', () => {
    it('should generate hooks without args correctly', async () => {
      ...
      try {
        const out = path.join(dir, 'index.ts')
        await runGenerator(hooks(openapiNoArgs, out, '../client', 'swr', { split: false }))
  ...
  describe('swr (path with special characters)', () => {
    it('should generate hooks for hyphenated paths', async () => {
      ...
      try {
        ...
        await runGenerator(hooks(hyphenOpenAPI, out, '../client', 'swr', { split: false }))
  ...
  describe('swr (path parameters)', () => {
    it('should generate hooks for path with parameters', async () => {
      ...
      try {
        ...
        await runGenerator(hooks(paramOpenAPI, out, '../client', 'swr', { split: false }))
  ...
  describe('swr (split mode with full CRUD resource)', () => {
    it('should generate split files for GET list, GET by id, POST, PUT, DELETE', async () => {
      ...
      try {
        ...
        expect(result).toStrictEqual(
          `Generated swr hooks written to ${path.join(dir, 'hooks')}/*.ts (index.ts included)`,
        )
...
export function getPutUsersIdKey() {
...
export function usePutUsersId<TError = unknown>(options?: {
  ...
  const { mutation: mutationOptions, options: clientOptions } = options ?? {}
  const { swrKey: customKey, ...restMutationOptions } = mutationOptions ?? {}
  const swrKey = customKey ?? getPutUsersIdKey()
  return {
    swrKey,
    ...useSWRMutation(
      swrKey,
  ...
  describe('swr (immutable mode)', () => {
    it('should generate both useSWR and useSWRImmutable hooks by default', async () => {
      ...
      try {
        const out = path.join(dir, 'index.ts')
        await expect(
          runGenerator(hooks(openapiImmutable, out, '../client', 'swr', { split: false })),
  ...
  describe('swr (enabled priority)', () => {
    it('should prioritize enabled over swrKey - enabled:false means no fetch even with swrKey', async () => {
      ...
      try {
        ...
        await runGenerator(hooks(simpleOpenAPI, out, '../client', 'swr', { split: false }))
  ...
  describe('swr (no pagination)', () => {
    it('should NOT generate Infinite hooks when x-pagination is absent', async () => {
      ...
      try {
        ...
        await runGenerator(hooks(openAPI, out, '../client', 'swr', { split: false }))
        const code = fs.readFileSync(out, 'utf-8')
...
import type { UseMutationOptions, DefaultError, QueryClient } from '@tanstack/react-query'
import type { ClientRequestOptions, InferRequestType } from 'hono/client'
import { parseResponse } from 'hono/client'
import { client } from '../client'
...
`
        ...
        expect(result).toStrictEqual(
          `Generated tanstack-query hooks written to ${path.join(dir, 'hooks')}/*.ts (index.ts included)`,
        )
      } finally {
        fs.rmSync(dir, { recursive: true, force: true })
      }
  ...
  describe('tanstackQuery (header parameters excluded from query key)', () => {
    it('omits header field from queryKey arg across query/suspense/infinite hooks', async () => {
      ...
      try {
        const out = path.join(dir, 'index.ts')
        await expect(
          runGenerator(hooks(spec, out, '../client', 'tanstack-query', { split: false })),
        ).resolves.toBeDefined()
        const code = fs.readFileSync(out, 'utf-8')
  ...
  describe('tanstack-query (query method)', () => {
    it('treats an OpenAPI 3.2 QUERY operation as a query hook named and keyed by its method', async () => {
      ...
      try {
        ...
        await runGenerator(hooks(openAPI, out, '../client', 'tanstack-query', { split: false }))
        const code = fs.readFileSync(out, 'utf-8')
        expect(code).toBe(`import {
  useQuery,
  useSuspenseQuery,
...
`
  ...
  describe('preactQuery (header parameters excluded from query key)', () => {
    it('omits header field from queryKey arg across query/suspense/infinite hooks', async () => {
      ...
      try {
        const out = path.join(dir, 'index.ts')
        await runGenerator(hooks(spec, out, '../client', 'preact-query', { split: false }))
        const code = fs.readFileSync(out, 'utf-8')
        expect(code).toBe(`import {
  useQuery,
  useSuspenseQuery,
...
import type { UndefinedInitialDataOptions, DefaultError, QueryClient } from '@tanstack/solid-query'
...
import type { CreateMutationOptions, DefaultError, QueryClient } from '@tanstack/solid-query'
import type { ClientRequestOptions, InferRequestType } from 'hono/client'
import { parseResponse } from 'hono/client'
import { client } from '../client'
...
`)
        ...
        expect(result).toStrictEqual(
          `Generated solid-query hooks written to ${path.join(dir, 'hooks')}/*.ts (index.ts included)`,
        )
      } finally {
        fs.rmSync(dir, { recursive: true, force: true })
      }
...
`
        ...
        expect(result).toStrictEqual(
          `Generated vue-query hooks written to ${path.join(dir, 'hooks')}/*.ts (index.ts included)`,
        )
      } finally {
        fs.rmSync(dir, { recursive: true, force: true })
      }
...
`
  ...
  describe('vueQuery (no args operations)', () => {
    it('should generate hooks without args correctly', async () => {
      ...
      try {
        const out = path.join(dir, 'index.ts')
        await runGenerator(hooks(openapiNoArgs, out, '../client', 'vue-query', { split: false }))
  ...
  describe('vueQuery (path with special characters)', () => {
    it('should generate hooks for hyphenated paths', async () => {
      ...
      try {
        ...
        await runGenerator(hooks(hyphenOpenAPI, out, '../client', 'vue-query', { split: false }))
  ...
  describe('vueQuery (path parameters)', () => {
    it('should generate hooks for path with parameters', async () => {
      ...
      try {
        ...
        await runGenerator(hooks(paramOpenAPI, out, '../client', 'vue-query', { split: false }))
...
`
  ...
  describe('vueQuery (split mode with full CRUD)', () => {
    it('should generate correct split files for full CRUD resource', async () => {
      ...
      try {
        const out = path.join(dir, 'hooks', 'index.ts')
        await runGenerator(hooks(openapiCrud, out, '../client', 'vue-query', { split: true }))
  ...
  describe('vueQuery (PUT/PATCH methods)', () => {
    it('should generate hooks for PUT and PATCH methods', async () => {
      ...
      try {
        ...
        await expect(
          runGenerator(hooks(putPatchOpenAPI, out, '../client', 'vue-query', { split: false })),
        ).resolves.toBeDefined()
...
`
  ...
  describe('vueQuery (header parameters excluded from query key)', () => {
    it('omits header field from queryKey arg and infinite queryKey arg', async () => {
      ...
      try {
        const out = path.join(dir, 'index.ts')
        await runGenerator(hooks(spec, out, '../client', 'vue-query', { split: false }))
        const code = fs.readFileSync(out, 'utf-8')
...
`
        ...
        const createPostPetsExpected = `import { createMutation, mutationOptions } from '@tanstack/svelte-query'
...
`
        ...
        const createPutPetsPetIdExpected = `import { createMutation, mutationOptions } from '@tanstack/svelte-query'
import type { CreateMutationOptions, DefaultError, QueryClient } from '@tanstack/svelte-query'
import type { ClientRequestOptions, InferRequestType } from 'hono/client'
import { parseResponse } from 'hono/client'
import { client } from '../client'
...
`
        ...
        const createDeletePetsPetIdExpected = `import { createMutation, mutationOptions } from '@tanstack/svelte-query'
import type { CreateMutationOptions, DefaultError, QueryClient } from '@tanstack/svelte-query'
import type { ClientRequestOptions, InferRequestType } from 'hono/client'
import { parseResponse } from 'hono/client'
import { client } from '../client'
...
`
        ...
        expect(result).toStrictEqual(
          `Generated svelte-query hooks written to ${path.join(dir, 'hooks')}/*.ts (index.ts included)`,
        )
      } finally {
        fs.rmSync(dir, { recursive: true, force: true })
      }
  ...
  describe('svelteQuery (path parameters)', () => {
    it('should generate hooks for path with parameters', async () => {
      ...
      try {
        ...
        await runGenerator(hooks(paramOpenAPI, out, '../client', 'svelte-query', { split: false }))
...
`
  ...
  describe('svelteQuery (split mode with full CRUD)', () => {
    it('should generate correct split files for full CRUD resource', async () => {
      ...
      try {
        const out = path.join(dir, 'hooks', 'index.ts')
        await runGenerator(hooks(openapiCrud, out, '../client', 'svelte-query', { split: true }))
...
`
        ...
        const postUsers = fs.readFileSync(path.join(dir, 'hooks', 'postUsers.ts'), 'utf-8')
        const postUsersExpected = `import { createMutation, mutationOptions } from '@tanstack/svelte-query'
import type { CreateMutationOptions, DefaultError, QueryClient } from '@tanstack/svelte-query'
import type { ClientRequestOptions, InferRequestType } from 'hono/client'
import { parseResponse } from 'hono/client'
import { client } from '../client'
...
`
        ...
        // Check PUT /users/{id} mutation file (path param, no query imports)
        const putUsersId = fs.readFileSync(path.join(dir, 'hooks', 'putUsersId.ts'), 'utf-8')
        const putUsersIdExpected = `import { createMutation, mutationOptions } from '@tanstack/svelte-query'
import type { CreateMutationOptions, DefaultError, QueryClient } from '@tanstack/svelte-query'
import type { ClientRequestOptions, InferRequestType } from 'hono/client'
import { parseResponse } from 'hono/client'
import { client } from '../client'
...
export function createPutUsersId<TError = DefaultError, TOnMutateResult = unknown>(
  options?: () => {
    mutation?: Omit<
      CreateMutationOptions<
        Awaited<
...
`
        ...
        // Check DELETE /users/{id} mutation file (204 with |undefined)
        const deleteUsersId = fs.readFileSync(path.join(dir, 'hooks', 'deleteUsersId.ts'), 'utf-8')
        const deleteUsersIdExpected = `import { createMutation, mutationOptions } from '@tanstack/svelte-query'
import type { CreateMutationOptions, DefaultError, QueryClient } from '@tanstack/svelte-query'
import type { ClientRequestOptions, InferRequestType } from 'hono/client'
import { parseResponse } from 'hono/client'
import { client } from '../client'
...
`
  ...
  describe('svelteQuery (header parameters excluded from query key)', () => {
    it('omits header field from queryKey arg in createQuery and infinite hook', async () => {
      ...
      try {
        const out = path.join(dir, 'index.ts')
        await runGenerator(hooks(spec, out, '../client', 'svelte-query', { split: false }))
        const code = fs.readFileSync(out, 'utf-8')
        expect(code).toBe(`import {
  createQuery,
  createInfiniteQuery,
...
`)
        ...
        const postUsers = fs.readFileSync(path.join(dir, 'hooks', 'postUsers.ts'), 'utf-8')
        expect(postUsers)
          .toBe(`import { injectMutation, mutationOptions } from '@tanstack/angular-query-experimental'
import type {
  CreateMutationOptions,
  DefaultError,
  InjectQueryOptions,
...
`)
        ...
        expect(result).toStrictEqual(
          `Generated angular-query hooks written to ${path.join(dir, 'hooks')}/*.ts (index.ts included)`,
        )
      } finally {
        fs.rmSync(dir, { recursive: true, force: true })
      }
