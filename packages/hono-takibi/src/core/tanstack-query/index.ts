...
const makeHookCode = (
  ...
  const keyDocs = [
    '/**',
    ` * Generates TanStack Query cache key for ${method.toUpperCase()} ${pathStr.replace(/\/\*/g, '/[*]')}`,
    ' */',
  ].join('\n')
  ...
  if (isQuery) {
    // useQuery hook for GET (TanStack Query pattern - Orval style)
    const argsSig = hasArgs ? `args:${inferRequestType},` : ''
    const queryOptionsType = `Omit<UseQueryOptions<${inferResponseType},Error>,'queryKey'|'queryFn'|'initialData'>`
...
/**
 * Generates the import header for TanStack Query hook files.
 */
const makeHeader = (
  importPath: string,
  hasQuery: boolean,
  ...
  const tanstackTypeImports: string[] = ['QueryClient']

  if (hasQuery) {
    tanstackImports.push('useQuery')
    tanstackTypeImports.push('UseQueryOptions')
  }
  if (hasMutation) {
    tanstackImports.push('useMutation')
    tanstackTypeImports.push('UseMutationOptions')
  }

  if (tanstackImports.length > 0) {
    lines.push(`import{${tanstackImports.join(',')}}from'@tanstack/react-query'`)
  }
  lines.push(`import type{${tanstackTypeImports.join(',')}}from'@tanstack/react-query'`)
...
> {
  ...
  if (!split) {
    ...
    return { ok: true, value: `Generated tanstack-query hooks written to ${output}` }
  }
  ...
  return {
    ok: true,
    value: `Generated tanstack-query hooks written to ${outDir}/*.ts (index.ts included)`,
  }
}
