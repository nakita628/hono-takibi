...
describe('takibi generate (sandbox)', () => {
  it('should generate Hono app with OpenAPI routes (no template/test)', async () => {
    ...
    try {
      ...
      expect(result).toBe(`🔥 Generated code written to ${out}`)
...
describe('basePath behavior', () => {
  it('basePath not specified: generates app without .basePath()', async () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'takibi-basepath-'))
    try {
...
`)
  ...
  it('basePath "/api": generates app with .basePath("/api")', async () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'takibi-basepath-'))
    try {
      const srcDir = path.join(dir, 'src')
      fs.mkdirSync(srcDir, { recursive: true })
...
`)
  ...
  it('basePath "/api/v1": generates app with .basePath("/api/v1")', async () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'takibi-basepath-'))
    try {
      const srcDir = path.join(dir, 'src')
      fs.mkdirSync(srcDir, { recursive: true })
...
describe('takibi error paths', () => {
  ...
  it('returns ok:false with the thrown message when zodOpenAPIHono throws', async () => {
    ...
    try {
      // Either the codegen fails, or it produces empty/odd-but-valid output. Both are
      // acceptable, but a failure has to carry a string message — not a bare `false`.
      const outcome = await runGenerator(
        takibi(malformed, path.join(dir, 'out.ts') as `${string}.ts`, errorPathOptions).pipe(
