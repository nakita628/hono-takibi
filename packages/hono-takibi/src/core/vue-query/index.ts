...
const makeHookCode = (
  ...
  const keyDocs = [
    '/**',
    ` * Generates Vue Query cache key for ${method.toUpperCase()} ${pathStr.replace(/\/\*/g, '/[*]')}`,
    ' */',
  ].join('\n')
  ...
  if (isQuery) {
    // useQuery hook for GET (Vue Query pattern - Svelte Query style without Omit)
    const argsSig = hasArgs ? `args:${inferRequestType},` : ''
    const queryOptionsType = `UseQueryOptions<${inferResponseType},Error>`
    ...
    // Spread queryOptions first, then override with queryKey and queryFn
    hookCode = `${docs}
export function ${hookName}(${argsSig}${optionsSig}){const{query:queryOptions,client:clientOptions}=options??{};const queryKey=${keyGetterCall};const query=useQuery({...queryOptions,queryKey,queryFn:async()=>parseResponse(${clientCall})},queryClient);return{...query,queryKey}}`
...
export function ${queryKeyGetterName}(){return['${honoPath}']as const}`
    ...
    // useMutation hook for POST/PUT/DELETE/PATCH (Vue Query pattern)
    ...
    if (hasArgs) {
      const mutationOptionsType = `UseMutationOptions<${inferResponseType}|undefined,Error,${inferRequestType}>`
      const optionsSig = `options?:{mutation?:${mutationOptionsType};client?:ClientRequestOptions},queryClient?:QueryClient`
      hookCode = `${docs}
export function ${hookName}(${optionsSig}){return useMutation<${inferResponseType}|undefined,Error,${inferRequestType}>({...options?.mutation,mutationFn:async(args)=>parseResponse(${deps.client}${pathResult.runtimePath}.$${method}(args,options?.client))},queryClient)}`
    } else {
      const mutationOptionsType = `UseMutationOptions<${inferResponseType}|undefined,Error,void>`
      const optionsSig = `options?:{mutation?:${mutationOptionsType};client?:ClientRequestOptions},queryClient?:QueryClient`
      hookCode = `${docs}
export function ${hookName}(${optionsSig}){return useMutation<${inferResponseType}|undefined,Error,void>({...options?.mutation,mutationFn:async()=>parseResponse(${deps.client}${pathResult.runtimePath}.$${method}(undefined,options?.client))},queryClient)}`
    }
...
/**
 * Generates the import header for Vue Query hook files.
 */
const makeHeader = (
  importPath: string,
  hasQuery: boolean,
  ...
  // Vue Query imports
  const vueQueryImports: string[] = []
  const vueQueryTypeImports: string[] = ['QueryClient']

  if (hasQuery) {
    vueQueryImports.push('useQuery')
    vueQueryTypeImports.push('UseQueryOptions')
  }
  if (hasMutation) {
    vueQueryImports.push('useMutation')
    vueQueryTypeImports.push('UseMutationOptions')
  }

  if (vueQueryImports.length > 0) {
    lines.push(`import{${vueQueryImports.join(',')}}from'@tanstack/vue-query'`)
  }
  lines.push(`import type{${vueQueryTypeImports.join(',')}}from'@tanstack/vue-query'`)
...
> {
  ...
  if (!split) {
    ...
    return { ok: true, value: `Generated vue-query hooks written to ${output}` }
  }
  ...
  return {
    ok: true,
    value: `Generated vue-query hooks written to ${outDir}/*.ts (index.ts included)`,
  }
}
