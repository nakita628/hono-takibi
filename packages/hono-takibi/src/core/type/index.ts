...
export async function type(openAPI: OpenAPI, output: `${string}.ts`): Promise<Result> {
  try {
    ...
    const routeNames = Array.from(
      new Set(
        Array.from(hono.matchAll(/export const ([A-Za-z_$][A-Za-z0-9_$]*Route)\b/g))
          .map((m) => m[1])
    ...
    if (routeNames.length === 0) {
      return { ok: false, error: 'No route constants (export const *Route) found in generated code.' }
    }

    const handlers = routeNames
      .map((routeName) => {
        const handlerName = `${routeName}Handler`
        return `export const ${handlerName}: RouteHandler<typeof ${routeName}> = (c) => c.json(undefined as any, 200 as any) as any`
      })
      .join('\n')

    const appInit =
      'export const api=app' + routeNames.map((r) => `.openapi(${r}, ${r}Handler)`).join('')
...
type CleanRoutes<T> =
  T extends OpenAPIHono<infer E, infer S, infer B>
    ? OpenAPIHono<E, RemoveIndexSignature<S>, B>
    : T

export type CleanedRoutes = CleanRoutes<typeof api>
`.trim() + '\n'

    const inferred = inferExportedType(virtualCode, 'CleanedRoutes')
    if (!inferred.ok) return inferred

    // ✅ sanitize しない（ユーザー環境に任せる）
    const dts =
      [
        ...
        `}`,
        ``,
        `declare const routes: ${inferred.value}`,
        `export default routes`,
        ``,
      ].join('\n')
...
function inferExportedType(code: string, exportName: string): Result {
  const VIRTUAL_FILE_NAME = 'virtual.ts'
  ...
  const checker = program.getTypeChecker()
  const moduleSymbol = checker.getSymbolAtLocation(sourceFile)
  if (!moduleSymbol) return { ok: false, error: 'Failed to get module symbol' }

  const sym = checker.getExportsOfModule(moduleSymbol).find((s) => s.getName() === exportName)
  if (!sym) return { ok: false, error: `Failed to find exported symbol: ${exportName}` }
  ...
  // ✅ UseFullyQualifiedType は付けない（物理パス import('/abs/...') を出しやすい）
  ...
  return { ok: true, value: typeString }
}
