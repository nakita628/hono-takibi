...
const generatePropertySchemaTestCases: { schema: Schema; expected: string }[] = [
  ...
  // number
  {
    ...
    expected: 'z.number().int().optional().openapi({example:1})',
  },
  // string
  {
    schema: { type: 'string', example: 'Dogs' },
    expected: 'z.string().optional().openapi({example:"Dogs"})',
  },
  {
    schema: { type: 'string', example: 'theUser' },
    expected: 'z.string().optional().openapi({example:"theUser"})',
  },
  // name
  {
    schema: { type: 'string', example: 'John' },
    expected: 'z.string().optional().openapi({example:"John"})',
  },
  {
    schema: { type: 'string', example: 'James' },
    expected: 'z.string().optional().openapi({example:"James"})',
  },
  // email
  {
    schema: { type: 'string', example: 'john@email.com' },
    expected: 'z.string().optional().openapi({example:"john@email.com"})',
  },
  // zip
  {
    schema: { type: 'string', example: '12345' },
    expected: 'z.string().optional().openapi({example:"12345"})',
  },
  // pet name
  {
    schema: { type: 'string', example: 'doggie' },
    expected: 'z.string().optional().openapi({example:"doggie"})',
  },
  // enum
  {
    schema: { type: 'string', enum: ['placed', 'approved', 'delivered'], example: 'approved' },
    expected: 'z.enum(["placed","approved","delivered"]).optional().openapi({example:"approved"})',
  },
  // date-time
  {
    schema: { type: 'string', format: 'date-time', example: '2023-01-15T09:30:00.000Z' },
    expected: 'z.string().datetime().optional().openapi({example:"2023-01-15T09:30:00.000Z"})',
  },
  // uuid
  {
