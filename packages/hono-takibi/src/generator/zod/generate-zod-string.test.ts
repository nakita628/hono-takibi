...
describe('generateZodStringSchema Test', () => {
  it.concurrent('generateZodString({}) -> z.string()', () => {
  ...
  it.concurrent('generateZodString({ minLength: 1 }) -> z.string().min(1)', () => {
    const result = generateZodString({ minLength: 1 })
    const expected = 'z.string().min(1)'
    expect(result).toBe(expected)
  })

  it.concurrent('generateZodString({ maxLength: 10 }) -> z.string().max(10)', () => {
  ...
  it.concurrent(`generateZodString({ format: 'email' }) -> z.string().email()`, () => {
    const result = generateZodString({ format: 'email' })
    const expected = 'z.string().email()'
    expect(result).toBe(expected)
  })

  it.concurrent(`generateZodString({ format: 'uuid' }) -> z.string().uuid()`, () => {
    const result = generateZodString({ format: 'uuid' })
    const expected = 'z.string().uuid()'
    expect(result).toBe(expected)
  })

  it.concurrent(`generateZodString({ default: 'test' }) -> z.string().default("test")`, () => {
    const result = generateZodString({
      default: 'test',
    })
  ...
  it.concurrent('generateZodString({ nullable: true }) -> z.string().nullable()', () => {
    const result = generateZodString({
      nullable: true,
    })
