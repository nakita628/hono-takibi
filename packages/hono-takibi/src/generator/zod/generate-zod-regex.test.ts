...
describe('generateZodRegex Test', () => {
  it.concurrent(`generateZodRegex('^[a-z]+$') -> .regex(/^[a-z]+$/)`, () => {
  ...
  it.concurrent(`generateZodRegex('^#[0-9a-fA-F]{6}$') -> .regex(/^#[0-9a-fA-F]{6}$/)`, () => {
  ...
  it.concurrent(`generateZodRegex('^\\d{2}/\\d{2}$') -> '.regex(/^\\d{2}\\/\\d{2}$/)'`, () => {
