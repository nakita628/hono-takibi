  ...
  // $ref
    ...
    describe('const', () => {
      ...
      it.concurrent('z.literal("test").nullable()', () => {
        expect(propertySchema({ const: 'fixed', nullable: true })).toBe('z.literal("fixed").nullable()')
      })
      it.concurrent('z.literal("test").nullable()', () => {
        expect(propertySchema({ type: ['null'], const: 'fixed' })).toBe('z.literal("fixed").nullable()')
      })
    })
    ...
    describe('enum', () => {
      ...
      it.concurrent('z.union([z.literal(1),z.literal(2),z.literal(3)])', () => {
        expect(propertySchema({ enum: [1, 2, 3] })).toBe('z.union([z.literal(1),z.literal(2),z.literal(3)])')
      })
      ...
      it.concurrent('z.union([z.literal(true),z.literal(false)])', () => {
        expect(propertySchema({ enum: [true, false] })).toBe('z.union([z.literal(true),z.literal(false)])')
      })
      it.concurrent('z.tuple([z.literal(1), z.literal(2)])', () => {
        expect(propertySchema({ type: 'array', enum: [[1, 2]] })).toBe('z.tuple([z.literal(1), z.literal(2)])')
      })
      it.concurrent(
    ...
    describe('string', () => {
      ...
      it.concurrent('z.string().min(1).max(10)', () => {
        expect(propertySchema({ type: 'string', minLength: 1, maxLength: 10 })).toBe('z.string().min(1).max(10)')
      })
      ...
      it.concurrent('z.length(5)', () => {
        expect(propertySchema({ type: 'string', minLength: 5, maxLength: 5 })).toBe('z.string().length(5)')
      })
    ...
    describe('integer', () => {
      ...
      it.concurrent('minimum: 100, exclusiveMinimum: true → z.int().gt(100)', () => {
        expect(propertySchema({ type: 'integer', minimum: 100, exclusiveMinimum: true })).toBe('z.int().gt(100)')
      })
      ...
      it.concurrent('maximum: 100, exclusiveMaximum: true → z.int().lt(100)', () => {
        expect(propertySchema({ type: 'integer', maximum: 100, exclusiveMaximum: true })).toBe('z.int().lt(100)')
      })
      ...
      it.concurrent('type: [integer, null], format: int32 → z.int32().nullable()', () => {
        expect(propertySchema({ type: ['integer', 'null'], format: 'int32' })).toBe('z.int32().nullable()')
      })
      // int32 positive
      it.concurrent('minimum: 0, exclusiveMinimum: true → z.int32().positive()', () => {
        expect(propertySchema({ type: 'integer', format: 'int32', minimum: 0, exclusiveMinimum: true })).toBe(
          'z.int32().positive()',
        )
      })
      // int32 nonnegative
      it.concurrent('minimum: 0, exclusiveMinimum: false → z.int32().nonnegative()', () => {
        expect(propertySchema({ type: 'integer', format: 'int32', minimum: 0, exclusiveMinimum: false })).toBe(
          'z.int32().nonnegative()',
        )
      })
      // int32 negative
      it.concurrent('maximum: 0, exclusiveMaximum: true → z.int32().negative()', () => {
        expect(propertySchema({ type: 'integer', format: 'int32', maximum: 0, exclusiveMaximum: true })).toBe(
          'z.int32().negative()',
        )
      })
      // int32 nonpositive
      it.concurrent('maximum: 0, exclusiveMaximum: false → z.int32().nonpositive()', () => {
        expect(propertySchema({ type: 'integer', format: 'int32', maximum: 0, exclusiveMaximum: false })).toBe(
          'z.int32().nonpositive()',
        )
      })
      // int32 min
      it.concurrent('minimum: 100 → z.int32().min(100)', () => {
        expect(propertySchema({ type: 'integer', format: 'int32', minimum: 100 })).toBe('z.int32().min(100)')
      })
      // int32 min 0
      it.concurrent('minimum: 0, exclusiveMinimum: true → z.int32().min(0)', () => {
        expect(propertySchema({ type: 'integer', format: 'int32', minimum: 0 })).toBe('z.int32().min(0)')
      })
      // int32 gt
      it.concurrent('minimum: 100, exclusiveMinimum: true → z.int32().gt(100)', () => {
        expect(propertySchema({ type: 'integer', format: 'int32', minimum: 100, exclusiveMinimum: true })).toBe(
          'z.int32().gt(100)',
        )
      })
      // int32 max
      it.concurrent('maximum: 100 → z.int32().max(100)', () => {
        expect(propertySchema({ type: 'integer', format: 'int32', maximum: 100 })).toBe('z.int32().max(100)')
      })
      // int32 max 0
      it.concurrent('maximum: 0, exclusiveMaximum: true → z.int32().max()', () => {
        expect(propertySchema({ type: 'integer', format: 'int32', maximum: 0 })).toBe('z.int32().max(0)')
      })
      // int32 lt
      it.concurrent('maximum: 100, exclusiveMaximum: true → z.int32().lt(100)', () => {
        expect(propertySchema({ type: 'integer', format: 'int32', maximum: 100, exclusiveMaximum: true })).toBe(
          'z.int32().lt(100)',
        )
      })
      // int32 multipleOf
      ...
      it.concurrent('default: 100 → z.int32().default(100)', () => {
        expect(propertySchema({ type: 'integer', format: 'int32', default: 100 })).toBe('z.int32().default(100)')
      })
      // int64
      ...
      it.concurrent('type: [integer, null], format: int64 → z.int64().nullable()', () => {
        expect(propertySchema({ type: ['integer', 'null'], format: 'int64' })).toBe('z.int64().nullable()')
      })
      // int64 positive
      it.concurrent('minimum: 0, exclusiveMinimum: true → z.int64().positive()', () => {
        expect(propertySchema({ type: 'integer', format: 'int64', minimum: 0, exclusiveMinimum: true })).toBe(
          'z.int64().positive()',
        )
      })
      // int64 nonnegative
      it.concurrent('minimum: 0, exclusiveMinimum: false → z.int64().nonnegative()', () => {
        expect(propertySchema({ type: 'integer', format: 'int64', minimum: 0, exclusiveMinimum: false })).toBe(
          'z.int64().nonnegative()',
        )
      })
      // int64 negative
      it.concurrent('maximum: 0, exclusiveMaximum: true → z.int64().negative()', () => {
        expect(propertySchema({ type: 'integer', format: 'int64', maximum: 0, exclusiveMaximum: true })).toBe(
          'z.int64().negative()',
        )
      })
      // int64 nonpositive
      it.concurrent('maximum: 0, exclusiveMaximum: false → z.int64().nonpositive()', () => {
        expect(propertySchema({ type: 'integer', format: 'int64', maximum: 0, exclusiveMaximum: false })).toBe(
          'z.int64().nonpositive()',
        )
      })
      // int64 min
      it.concurrent('minimum: 100 → z.int64().min(100n)', () => {
        expect(propertySchema({ type: 'integer', format: 'int64', minimum: 100 })).toBe('z.int64().min(100n)')
      })
      // int64 min 0
      it.concurrent('minimum: 0, exclusiveMinimum: true → z.int64().min(0n)', () => {
        expect(propertySchema({ type: 'integer', format: 'int64', minimum: 0 })).toBe('z.int64().min(0n)')
      })
      // int64 gt
      it.concurrent('minimum: 100, exclusiveMinimum: true → z.int64().gt(100n)', () => {
        expect(propertySchema({ type: 'integer', format: 'int64', minimum: 100, exclusiveMinimum: true })).toBe(
          'z.int64().gt(100n)',
        )
      })
      // int64 max
      it.concurrent('maximum: 100 → z.int64().max(100n)', () => {
        expect(propertySchema({ type: 'integer', format: 'int64', maximum: 100 })).toBe('z.int64().max(100n)')
      })
      // int64 max 0
      it.concurrent('maximum: 0, exclusiveMaximum: true → z.int64().negative()', () => {
        expect(propertySchema({ type: 'integer', format: 'int64', maximum: 0 })).toBe('z.int64().max(0n)')
      })
      // int64 lt
      it.concurrent('maximum: 100, exclusiveMaximum: true → z.int64().lt(100n)', () => {
        expect(propertySchema({ type: 'integer', format: 'int64', maximum: 100, exclusiveMaximum: true })).toBe(
          'z.int64().lt(100n)',
        )
      })
      // int64 multipleOf
      ...
      it.concurrent('type: [integer, null], format: bigint → z.bigint().nullable()', () => {
        expect(propertySchema({ type: ['integer', 'null'], format: 'bigint' })).toBe('z.bigint().nullable()')
      })
      // bigint positive
      it.concurrent('minimum: 0, exclusiveMinimum: true → z.bigint().positive()', () => {
        expect(propertySchema({ type: 'integer', format: 'bigint', minimum: 0, exclusiveMinimum: true })).toBe(
          'z.bigint().positive()',
        )
      })
      // bigint nonnegative
      it.concurrent('minimum: 0, exclusiveMinimum: false → z.bigint().nonnegative()', () => {
        expect(propertySchema({ type: 'integer', format: 'bigint', minimum: 0, exclusiveMinimum: false })).toBe(
          'z.bigint().nonnegative()',
        )
      })
      // bigint negative
      it.concurrent('maximum: 0, exclusiveMaximum: true → z.bigint().negative()', () => {
        expect(propertySchema({ type: 'integer', format: 'bigint', maximum: 0, exclusiveMaximum: true })).toBe(
          'z.bigint().negative()',
        )
      })
      // bigint nonpositive
      it.concurrent('z.bigint() -> .nonpositive()', () => {
        expect(propertySchema({ type: 'integer', format: 'bigint', maximum: 0, exclusiveMaximum: false })).toBe(
      ...
      it.concurrent('minimum: 100, exclusiveMinimum: true → z.bigint().gt(BigInt(100))', () => {
        expect(propertySchema({ type: 'integer', format: 'bigint', minimum: 100, exclusiveMinimum: true })).toBe(
      ...
      it.concurrent('maximum: 100, exclusiveMaximum: true → z.bigint().lt(BigInt(100))', () => {
        expect(propertySchema({ type: 'integer', format: 'bigint', maximum: 100, exclusiveMaximum: true })).toBe(
    ...
    describe('array', () => {
      ...
      it.concurrent('z.array(z.string())', () => {
        expect(propertySchema({ type: 'array', items: { type: 'string' } })).toBe('z.array(z.string())')
      })
      it.concurrent('z.array(z.number())', () => {
        expect(propertySchema({ type: 'array', items: { type: 'number' } })).toBe('z.array(z.number())')
      })
      it.concurrent('z.array(z.boolean())', () => {
        expect(propertySchema({ type: 'array', items: { type: 'boolean' } })).toBe('z.array(z.boolean())')
      })
      it.concurrent('z.array(z.object({}))', () => {
        expect(propertySchema({ type: 'array', items: { type: 'object' } })).toBe('z.array(z.object({}))')
      })
      ...
      it.concurrent('z.array(z.string()).min(1).max(10)', () => {
        expect(propertySchema({ type: 'array', items: { type: 'string' }, minItems: 1, maxItems: 10 })).toBe(
          'z.array(z.string()).min(1).max(10)',
        )
      })
      it.concurrent('z.array(z.string()).length(5)', () => {
        expect(propertySchema({ type: 'array', items: { type: 'string' }, minItems: 5, maxItems: 5 })).toBe(
    ...
    describe('null', () => {
      it.concurrent('z.null()', () => {
        expect(propertySchema({ type: 'null' })).toBe('z.null()')
      })
    })
