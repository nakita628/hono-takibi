...
describe('zodToOpenAPI', () => {
  ...
  describe('const', () => {
    ...
    it.concurrent('z.literal("test").nullable()', () => {
      expect(zodToOpenAPI({ const: 'fixed', nullable: true })).toBe('z.literal("fixed").nullable()')
    })
    it.concurrent('z.literal("test").nullable()', () => {
      expect(zodToOpenAPI({ type: ['null'], const: 'fixed' })).toBe('z.literal("fixed").nullable()')
    })
  })
  ...
  describe('enum', () => {
    it.concurrent('z.enum(["A","B"])', () => {
      expect(zodToOpenAPI({ enum: ['A', 'B'], type: 'string' })).toBe('z.enum(["A","B"])')
    })
    // nullable
    it.concurrent('z.enum(["A","B"]).nullable()', () => {
      expect(zodToOpenAPI({ enum: ['A', 'B'], type: ['string'], nullable: true })).toBe(
        'z.enum(["A","B"]).nullable()',
      )
    })
    ...
    it.concurrent(`z.literal('test')`, () => {
      expect(zodToOpenAPI({ enum: ['test'] })).toBe(`z.literal('test')`)
    })
    it.concurrent('z.enum(["a","b","c"])', () => {
      expect(zodToOpenAPI({ enum: ['a', 'b', 'c'] })).toBe('z.enum(["a","b","c"])')
    })
    ...
    it.concurrent('z.union([z.literal(1),z.literal(2),z.literal(3)])', () => {
      expect(zodToOpenAPI({ enum: [1, 2, 3] })).toBe(
        'z.union([z.literal(1),z.literal(2),z.literal(3)])',
      )
    })
    it.concurrent('z.literal(true)', () => {
      expect(zodToOpenAPI({ enum: [true] })).toBe('z.literal(true)')
    })
    ...
    it.concurrent("z.union([z.literal(1),z.literal('two'),z.null()])", () => {
      expect(zodToOpenAPI({ enum: [1, 'two', null] })).toBe(
  ...
  describe('string', () => {
    ...
    it.concurrent('z.string().regex(/^[a-zA-Z]+$/)', () => {
      expect(zodToOpenAPI({ type: 'string', pattern: '^[a-zA-Z]+$' })).toBe(
  ...
  describe('number', () => {
    ...
    it.concurrent('maximum: 0, exclusiveMaximum: true → z.number().negative()', () => {
      expect(zodToOpenAPI({ type: 'number', maximum: 0, exclusiveMaximum: true })).toBe(
        'z.number().negative()',
      )
    })
    // nonpositive
    it.concurrent('maximum: 0 → z.number().nonpositive()', () => {
      expect(zodToOpenAPI({ type: 'number', maximum: 0, exclusiveMaximum: false })).toBe(
    ...
    it.concurrent('minimum: 100, exclusiveMinimum: true → z.number().gt(100)', () => {
      expect(zodToOpenAPI({ type: 'number', minimum: 100, exclusiveMinimum: true })).toBe(
        'z.number().gt(100)',
      )
    })
    // max
    ...
    it.concurrent('maximum: 100, exclusiveMaximum: true → z.number().lt(100)', () => {
      expect(zodToOpenAPI({ type: 'number', maximum: 100, exclusiveMaximum: true })).toBe(
        'z.number().lt(100)',
      )
    })
    // multipleOf
  ...
  describe('integer', () => {
    ...
    it.concurrent('minimum: 0, exclusiveMinimum: true → z.int().positive()', () => {
      expect(zodToOpenAPI({ type: 'integer', minimum: 0, exclusiveMinimum: true })).toBe(
        'z.int().positive()',
      )
    })
    // nonnegative
    it.concurrent('minimum: 0, exclusiveMinimum: false → z.int().nonnegative()', () => {
      expect(zodToOpenAPI({ type: 'integer', minimum: 0, exclusiveMinimum: false })).toBe(
        'z.int().nonnegative()',
      )
    })
    // negative
    it.concurrent('maximum: 0, exclusiveMaximum: true → z.int().negative()', () => {
      expect(zodToOpenAPI({ type: 'integer', maximum: 0, exclusiveMaximum: true })).toBe(
        'z.int().negative()',
      )
    })
    // nonpositive
    it.concurrent('maximum: 0, exclusiveMaximum: false → z.int().nonpositive()', () => {
      expect(zodToOpenAPI({ type: 'integer', maximum: 0, exclusiveMaximum: false })).toBe(
        'z.int().nonpositive()',
      )
    })
    // min
    ...
    it.concurrent('minimum: 100, exclusiveMinimum: true → z.int().gt(100)', () => {
      expect(zodToOpenAPI({ type: 'integer', minimum: 100, exclusiveMinimum: true })).toBe(
        'z.int().gt(100)',
      )
    })
    it.concurrent('exclusiveMinimum: 100 → z.int().gt(100)', () => {
      expect(zodToOpenAPI({ type: 'integer', exclusiveMinimum: 100 })).toBe('z.int().gt(100)')
    })
    // max
    ...
    it.concurrent('maximum: 100, exclusiveMaximum: true → z.int().lt(100)', () => {
      expect(zodToOpenAPI({ type: 'integer', maximum: 100, exclusiveMaximum: true })).toBe(
        'z.int().lt(100)',
      )
    })
    ...
    it.concurrent('type: [integer, null], format: int32 → z.int32().nullable()', () => {
      expect(zodToOpenAPI({ type: ['integer', 'null'], format: 'int32' })).toBe(
        'z.int32().nullable()',
      )
    })
    // int32 positive
    it.concurrent('minimum: 0, exclusiveMinimum: true → z.int32().positive()', () => {
      expect(
        zodToOpenAPI({ type: 'integer', format: 'int32', minimum: 0, exclusiveMinimum: true }),
      ).toBe('z.int32().positive()')
    })
    // int32 nonnegative
    it.concurrent('minimum: 0, exclusiveMinimum: false → z.int32().nonnegative()', () => {
      expect(
        zodToOpenAPI({ type: 'integer', format: 'int32', minimum: 0, exclusiveMinimum: false }),
      ).toBe('z.int32().nonnegative()')
    })
    // int32 negative
    it.concurrent('maximum: 0, exclusiveMaximum: true → z.int32().negative()', () => {
      expect(
        zodToOpenAPI({ type: 'integer', format: 'int32', maximum: 0, exclusiveMaximum: true }),
      ).toBe('z.int32().negative()')
    })
    // int32 nonpositive
    it.concurrent('maximum: 0, exclusiveMaximum: false → z.int32().nonpositive()', () => {
      expect(
        zodToOpenAPI({ type: 'integer', format: 'int32', maximum: 0, exclusiveMaximum: false }),
      ).toBe('z.int32().nonpositive()')
    })
    // int32 min
    it.concurrent('minimum: 100 → z.int32().min(100)', () => {
      expect(zodToOpenAPI({ type: 'integer', format: 'int32', minimum: 100 })).toBe(
        'z.int32().min(100)',
      )
    })
    // int32 min 0
    it.concurrent('minimum: 0, exclusiveMinimum: true → z.int32().min(0)', () => {
      expect(zodToOpenAPI({ type: 'integer', format: 'int32', minimum: 0 })).toBe(
        'z.int32().min(0)',
      )
    })
    // int32 gt
    it.concurrent('minimum: 100, exclusiveMinimum: true → z.int32().gt(100)', () => {
      expect(
        zodToOpenAPI({ type: 'integer', format: 'int32', minimum: 100, exclusiveMinimum: true }),
      ).toBe('z.int32().gt(100)')
    })
    // int32 max
    it.concurrent('maximum: 100 → z.int32().max(100)', () => {
      expect(zodToOpenAPI({ type: 'integer', format: 'int32', maximum: 100 })).toBe(
        'z.int32().max(100)',
      )
    })
    // int32 max 0
    it.concurrent('maximum: 0, exclusiveMaximum: true → z.int32().max()', () => {
      expect(zodToOpenAPI({ type: 'integer', format: 'int32', maximum: 0 })).toBe(
        'z.int32().max(0)',
      )
    })
    // int32 lt
    it.concurrent('maximum: 100, exclusiveMaximum: true → z.int32().lt(100)', () => {
      expect(
        zodToOpenAPI({ type: 'integer', format: 'int32', maximum: 100, exclusiveMaximum: true }),
      ).toBe('z.int32().lt(100)')
    })
    // int32 multipleOf
    it.concurrent('type: integer, format: int32, multipleOf: 2 → z.int32().multipleOf(2)', () => {
      expect(zodToOpenAPI({ type: 'integer', format: 'int32', multipleOf: 2 })).toBe(
        'z.int32().multipleOf(2)',
      )
    })
    // int32 default
    it.concurrent('default: 100 → z.int32().default(100)', () => {
      expect(zodToOpenAPI({ type: 'integer', format: 'int32', default: 100 })).toBe(
        'z.int32().default(100)',
      )
    })
    // int64
    ...
    it.concurrent('type: [integer, null], format: int64 → z.int64().nullable()', () => {
      expect(zodToOpenAPI({ type: ['integer', 'null'], format: 'int64' })).toBe(
        'z.int64().nullable()',
      )
    })
    // int64 positive
    it.concurrent('minimum: 0, exclusiveMinimum: true → z.int64().positive()', () => {
      expect(
        zodToOpenAPI({ type: 'integer', format: 'int64', minimum: 0, exclusiveMinimum: true }),
      ).toBe('z.int64().positive()')
    })
    // int64 nonnegative
    it.concurrent('minimum: 0, exclusiveMinimum: false → z.int64().nonnegative()', () => {
      expect(
        zodToOpenAPI({ type: 'integer', format: 'int64', minimum: 0, exclusiveMinimum: false }),
      ).toBe('z.int64().nonnegative()')
    })
    // int64 negative
    it.concurrent('maximum: 0, exclusiveMaximum: true → z.int64().negative()', () => {
      expect(
        zodToOpenAPI({ type: 'integer', format: 'int64', maximum: 0, exclusiveMaximum: true }),
      ).toBe('z.int64().negative()')
    })
    // int64 nonpositive
    it.concurrent('maximum: 0, exclusiveMaximum: false → z.int64().nonpositive()', () => {
      expect(
        zodToOpenAPI({ type: 'integer', format: 'int64', maximum: 0, exclusiveMaximum: false }),
      ).toBe('z.int64().nonpositive()')
    })
    // int64 min
    it.concurrent('minimum: 100 → z.int64().min(100n)', () => {
      expect(zodToOpenAPI({ type: 'integer', format: 'int64', minimum: 100 })).toBe(
        'z.int64().min(100n)',
      )
    })
    // int64 min 0
    it.concurrent('minimum: 0, exclusiveMinimum: true → z.int64().min(0n)', () => {
      expect(zodToOpenAPI({ type: 'integer', format: 'int64', minimum: 0 })).toBe(
        'z.int64().min(0n)',
      )
    })
    // int64 gt
    it.concurrent('minimum: 100, exclusiveMinimum: true → z.int64().gt(100n)', () => {
      expect(
        zodToOpenAPI({ type: 'integer', format: 'int64', minimum: 100, exclusiveMinimum: true }),
      ).toBe('z.int64().gt(100n)')
    })
    // int64 max
    it.concurrent('maximum: 100 → z.int64().max(100n)', () => {
      expect(zodToOpenAPI({ type: 'integer', format: 'int64', maximum: 100 })).toBe(
        'z.int64().max(100n)',
      )
    })
    // int64 max 0
    it.concurrent('maximum: 0, exclusiveMaximum: true → z.int64().negative()', () => {
      expect(zodToOpenAPI({ type: 'integer', format: 'int64', maximum: 0 })).toBe(
        'z.int64().max(0n)',
      )
    })
    // int64 lt
    it.concurrent('maximum: 100, exclusiveMaximum: true → z.int64().lt(100n)', () => {
      expect(
        zodToOpenAPI({ type: 'integer', format: 'int64', maximum: 100, exclusiveMaximum: true }),
      ).toBe('z.int64().lt(100n)')
    })
    // int64 multipleOf
    it.concurrent('type: integer, format: int64, multipleOf: 2 → z.int64().multipleOf(2n)', () => {
      expect(zodToOpenAPI({ type: 'integer', format: 'int64', multipleOf: 2 })).toBe(
        'z.int64().multipleOf(2n)',
      )
    })
    // int64 default
    it.concurrent('default: 100 → z.int64().default(100n)', () => {
      expect(zodToOpenAPI({ type: 'integer', format: 'int64', default: 100 })).toBe(
    ...
    it.concurrent('type: [integer, null], format: bigint → z.bigint().nullable()', () => {
      expect(zodToOpenAPI({ type: ['integer', 'null'], format: 'bigint' })).toBe(
        'z.bigint().nullable()',
      )
    })
    // bigint positive
    it.concurrent('minimum: 0, exclusiveMinimum: true → z.bigint().positive()', () => {
      expect(
        zodToOpenAPI({ type: 'integer', format: 'bigint', minimum: 0, exclusiveMinimum: true }),
      ).toBe('z.bigint().positive()')
    })
    // bigint nonnegative
    it.concurrent('minimum: 0, exclusiveMinimum: false → z.bigint().nonnegative()', () => {
      expect(
        zodToOpenAPI({ type: 'integer', format: 'bigint', minimum: 0, exclusiveMinimum: false }),
      ).toBe('z.bigint().nonnegative()')
    })
    // bigint negative
    it.concurrent('maximum: 0, exclusiveMaximum: true → z.bigint().negative()', () => {
      expect(
        zodToOpenAPI({ type: 'integer', format: 'bigint', maximum: 0, exclusiveMaximum: true }),
      ).toBe('z.bigint().negative()')
    })
    // bigint nonpositive
    it.concurrent('z.bigint() -> .nonpositive()', () => {
      expect(
        zodToOpenAPI({ type: 'integer', format: 'bigint', maximum: 0, exclusiveMaximum: false }),
      ).toBe('z.bigint().nonpositive()')
    })
    // bigint min
    it.concurrent('minimum: 100 → z.bigint().min(BigInt(100))', () => {
      expect(zodToOpenAPI({ type: 'integer', format: 'bigint', minimum: 100 })).toBe(
        'z.bigint().min(BigInt(100))',
      )
    })
    // bigint min 0
    it.concurrent('minimum: 0, exclusiveMinimum: true → z.bigint().min(BigInt(0))', () => {
      expect(zodToOpenAPI({ type: 'integer', format: 'bigint', minimum: 0 })).toBe(
    ...
    it.concurrent('minimum: 100, exclusiveMinimum: true → z.bigint().gt(BigInt(100))', () => {
      expect(
        zodToOpenAPI({ type: 'integer', format: 'bigint', minimum: 100, exclusiveMinimum: true }),
      ).toBe('z.bigint().gt(BigInt(100))')
    })
    // bigint max
    it.concurrent('maximum: 100 → z.bigint().max(BigInt(100))', () => {
      expect(zodToOpenAPI({ type: 'integer', format: 'bigint', maximum: 100 })).toBe(
    ...
    it.concurrent('maximum: 100, exclusiveMaximum: true → z.bigint().lt(BigInt(100))', () => {
      expect(
        zodToOpenAPI({ type: 'integer', format: 'bigint', maximum: 100, exclusiveMaximum: true }),
      ).toBe('z.bigint().lt(BigInt(100))')
    })
    // bigint max 0
    it.concurrent('maximum: 0, exclusiveMaximum: true → z.bigint().max(BigInt(0))', () => {
      expect(zodToOpenAPI({ type: 'integer', format: 'bigint', maximum: 0 })).toBe(
    ...
    it.concurrent(
      ...
      () => {
        expect(zodToOpenAPI({ type: 'integer', format: 'bigint', multipleOf: 2 })).toBe(
    ...
    it.concurrent('default: 100 → z.bigint().default(BigInt(100))', () => {
      expect(zodToOpenAPI({ type: 'integer', format: 'bigint', default: 100 })).toBe(
  ...
  describe('array', () => {
    it.concurrent('z.array(z.string()).nullable()', () => {
      expect(zodToOpenAPI({ type: 'array', nullable: true, items: { type: 'string' } })).toBe(
        'z.array(z.string()).nullable()',
      )
    })
    it.concurrent('z.array(z.string())', () => {
      expect(zodToOpenAPI({ type: 'array', items: { type: 'string' } })).toBe('z.array(z.string())')
    })
    it.concurrent('z.array(z.number())', () => {
      expect(zodToOpenAPI({ type: 'array', items: { type: 'number' } })).toBe('z.array(z.number())')
    })
    it.concurrent('z.array(z.boolean())', () => {
      expect(zodToOpenAPI({ type: 'array', items: { type: 'boolean' } })).toBe(
        'z.array(z.boolean())',
      )
    })
    it.concurrent('z.array(z.object({}))', () => {
      expect(zodToOpenAPI({ type: 'array', items: { type: 'object' } })).toBe(
        'z.array(z.object({}))',
      )
    })
    ...
    it.concurrent('z.array(z.string()).min(1)', () => {
      expect(zodToOpenAPI({ type: 'array', items: { type: 'string' }, minItems: 1 })).toBe(
        'z.array(z.string()).min(1)',
      )
    })
    it.concurrent('z.array(z.string()).max(10)', () => {
      expect(zodToOpenAPI({ type: 'array', items: { type: 'string' }, maxItems: 10 })).toBe(
    ...
    it.concurrent('z.array(z.string()).length(5)', () => {
      expect(
        zodToOpenAPI({ type: 'array', items: { type: 'string' }, minItems: 5, maxItems: 5 }),
  ...
  describe('date', () => {
    ...
    it.concurrent('z.date().nullable()', () => {
      expect(zodToOpenAPI({ type: 'date', nullable: true })).toBe('z.date().nullable()')
    })
    it.concurrent('z.date().nullable() if type: ["date", "null"]', () => {
      expect(zodToOpenAPI({ type: ['date', 'null'] })).toBe('z.date().nullable()')
    })
  })
  ...
  describe('any', () => {
    it.concurrent('z.any()', () => {
      expect(zodToOpenAPI({})).toBe('z.any()')
    })
  })
})
