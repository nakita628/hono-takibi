...
export function request(
  ...
  if (requestBodyContentTypes.length > 0 && resolvedBody?.content) {
    const contentWithResolvedExamples = (() => {
      ...
      const out: typeof resolvedBody.content = {}
      for (const contentType of Object.keys(resolvedBody.content)) {
        const media = resolvedBody.content[contentType]
        ...
        const resolvedExamples: NonNullable<typeof media.examples> = {}
        for (const exampleKey of Object.keys(examples)) {
          ...
          if (isRef(example) && example.$ref.startsWith('#/components/examples/')) {
            ...
            resolvedExamples[exampleKey] = resolved ?? example
            continue
          }
          resolvedExamples[exampleKey] = example
        }

        out[contentType] = { ...media, examples: resolvedExamples }
      }
      return out
    })()
