...
// Test run
// pnpm vitest run ./src/helper/schema.test.ts

describe('findSchemaRefs', () => {
  it('should find schema references in code', () => {
  ...
  it('should handle code with no schema refs', () => {
...
describe('makeSchemaInfo', () => {
  ...
  it('should create basic schema info', () => {
...
describe('makeSchemaCode', () => {
  ...
  it('should generate schema code with export', () => {
  ...
  it('should generate schema code with type export', () => {
    const analysis = analyzeCircularSchemas(schemas, ['User'])
    const info = makeSchemaInfo('User', schemas.User, analysis)
  ...
  it('should generate schema code without export keyword', () => {
    const analysis = analyzeCircularSchemas(schemas, ['User'])
    const info = makeSchemaInfo('User', schemas.User, analysis)
...
describe('makeSplitSchemaFile', () => {
  describe('cyclic group type references', () => {
    ...
    it('should use z.infer for cyclic group refs in type definition', () => {
    ...
    it('should generate correct imports without type imports', () => {
      ...
      // Should NOT import types (they would cause name conflicts)
    ...
    it('should export z.infer type at the end', () => {
      const schemaNames = Object.keys(schemas)
      const analysis = analyzeCircularSchemas(schemas, schemaNames)
      const result = makeSplitSchemaFile('Event', schemas.Event, schemas, analysis, true)
  ...
  describe('non-cyclic schemas', () => {
    ...
    it('should import schema for refs in non-cyclic schemas', () => {
      const schemaNames = Object.keys(schemas)
      const analysis = analyzeCircularSchemas(schemas, schemaNames)
      const result = makeSplitSchemaFile('Profile', schemas.Profile, schemas, analysis, true)

      const lines = result.split('\n')
      // Should have import for UserSchema
    ...
    it('should not generate type definition for non-cyclic schemas', () => {
      const schemaNames = Object.keys(schemas)
      const analysis = analyzeCircularSchemas(schemas, schemaNames)
      const result = makeSplitSchemaFile('Profile', schemas.Profile, schemas, analysis, true)

      // Non-cyclic schemas should NOT have a type definition block
      expect(result.includes('type ProfileType')).toBe(false)
    })

    it('should generate exact output for simple non-cyclic schema', () => {
      const schemaNames = Object.keys(schemas)
      const analysis = analyzeCircularSchemas(schemas, schemaNames)
      const result = makeSplitSchemaFile('Profile', schemas.Profile, schemas, analysis, true)
...
describe('makeTypeDefinition', () => {
  ...
  it('should generate type definition with z.infer for non-self refs', () => {
    const schemaNames = Object.keys(schemas)
    const analysis = analyzeCircularSchemas(schemas, schemaNames)
  ...
  it('should generate type definition for self-referencing schema', () => {
