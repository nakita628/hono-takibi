...
// Test run
// pnpm vitest run ./packages/hono-takibi/src/helper/schema.ts
if (import.meta.vitest) {
  const { describe, it, expect } = import.meta.vitest
  const { analyzeCircularSchemas } = await import('./ast.js')
  ...
  describe('makeSplitSchemaFile', () => {
    describe('cyclic group type references', () => {
      ...
      it.concurrent('should generate correct imports without type imports', () => {
        ...
        expect(
          importLines.some((line) => line === "import{EventTypeSchema}from'./eventType'"),
        ).toBe(true)
        expect(
