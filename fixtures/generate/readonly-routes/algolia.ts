...
type FacetFiltersType = readonly FacetFiltersType[] | string

type OptionalFiltersType = readonly OptionalFiltersType[] | string

type NumericFiltersType = readonly NumericFiltersType[] | string

type TagFiltersType = readonly TagFiltersType[] | string

type ReRankingApplyFilterType = readonly ReRankingApplyFilterType[] | string
...
type HighlightResultOptionType = {
  readonly value: z.infer<typeof HighlightedValueSchema>
  readonly matchLevel: z.infer<typeof MatchLevelSchema>
...
const HighlightResultOptionSchema: z.ZodType<HighlightResultOptionType> = z
  .strictObject({
    ...
    matchedWords: z.array(z.string()).openapi({
...
const HighlightResultArraySchema: z.ZodType<HighlightResultArrayType> = z
  .lazy(() =>
    z.array(HighlightResultSchema).openapi({
...
type HighlightResultArrayType = readonly z.infer<typeof HighlightResultSchema>[]
...
const SnippetResultArraySchema: z.ZodType<SnippetResultArrayType> = z
  .lazy(() =>
    z.array(SnippetResultSchema).openapi({
...
type SnippetResultArrayType = readonly z.infer<typeof SnippetResultSchema>[]
...
const RedirectRuleIndexMetadataSchema = z
  .object({
    ...
    data: z.object({ ruleObjectID: z.string() }).openapi({
...
const BaseSearchResponseSchema = z
  .looseObject({
    abTestID: z.int().exactOptional().openapi({
      description:
        'A/B test ID. This is only included in the response for indices that are part of an A/B test.',
    }),
    abTestVariantID: z.int().min(1).exactOptional().openapi({
      description:
    ...
    exhaustive: z
      .object({
        facetsCount: z.boolean().exactOptional().openapi({
          title: 'facetsCount',
          description:
    ...
    serverTimeMS: z.int().exactOptional().openapi({
      description: 'Time the server took to process the request, in milliseconds.',
      example: 20,
    }),
    serverUsed: z.string().exactOptional().openapi({
    ...
    queryID: z.string().exactOptional().openapi({
      description:
        'Unique identifier for the query. This is used for [click analytics](https://www.algolia.com/doc/guides/analytics/click-analytics).',
      example: 'a00dbc80a8d13c4565a442e7e2dca80a',
    }),
    _automaticInsights: z.boolean().exactOptional().openapi({
...
const MatchedGeoLocationSchema = z
  .object({
    ...
    distance: z.int().exactOptional().openapi({
...
const RankingInfoSchema = z
  .strictObject({
    ...
    firstMatchedWord: z.int().min(0).openapi({
      description:
        'Position of the first matched word in the best matching attribute of the record.',
    }),
    geoDistance: z.int().min(0).openapi({
      description:
    ...
    proximityDistance: z.int().min(0).exactOptional().openapi({
      description:
        'Number of words between multiple matches in the query plus 1. For single word queries, `proximityDistance` is 0.',
    }),
    userScore: z.int().openapi({
      description:
...
const DistinctSeqIDSchema = z.int().readonly().openapi('DistinctSeqID')
...
const SearchHitsSchema = z
  .looseObject({
    hits: z.array(HitSchema).openapi({
      description:
...
const BaseIndexSettingsSchema = z
  .strictObject({
    ...
    paginationLimitedTo: z.int().max(20000).default(1000).exactOptional().openapi({
      example: 100,
      description:
    ...
    attributeForDistinct: z.string().exactOptional().openapi({
      description:
...
const TypoToleranceSchema = z
  .xor([
    z.boolean().default(true).openapi({
      description:
...
const BooleanStringSchema = z.enum(['true', 'false']).readonly().openapi('BooleanString')

const IgnorePluralsSchema = z
  .xor([
    z.array(SupportedLanguageSchema).openapi({
      description:
        'ISO code for languages for which this feature should be active.\nThis overrides languages you set with `queryLanguages`.\n',
    }),
    BooleanStringSchema,
    z.boolean().default(false).openapi({
      description:
...
const DistinctSchema = z
  .xor([
    ...
    z.int().min(0).max(4).default(0).openapi({
      description:
...
const ApiKeySchema = z
  .strictObject({
    ...
    description: z.string().default('').exactOptional().openapi({
    ...
    maxHitsPerQuery: z.int().default(0).exactOptional().openapi({
      description:
        "Maximum number of results this API key can retrieve in one query.\nBy default, there's no limit.\n",
    }),
    maxQueriesPerIPPerHour: z.int().default(0).exactOptional().openapi({
      description:
    ...
    queryParameters: z.string().default('').exactOptional().openapi({
      description:
    ...
    validity: z.int().default(0).exactOptional().openapi({
      description:
...
const FacetFiltersSchema: z.ZodType<FacetFiltersType> = z
  .lazy(() =>
    z.xor([z.array(FacetFiltersSchema), z.string()]).openapi({
      description:
...
const OptionalFiltersSchema: z.ZodType<OptionalFiltersType> = z
  .lazy(() =>
    z.xor([z.array(OptionalFiltersSchema), z.string()]).openapi({
      description:
...
const NumericFiltersSchema: z.ZodType<NumericFiltersType> = z
  .lazy(() =>
    z.xor([z.array(NumericFiltersSchema), z.string()]).openapi({
      description:
...
const TagFiltersSchema: z.ZodType<TagFiltersType> = z
  .lazy(() =>
    z.xor([z.array(TagFiltersSchema), z.string()]).openapi({
      description:
...
const AroundPrecisionFromValueSchema = z
  .array(
    z
      .object({
        from: z.int().exactOptional().openapi({
          description:
            'Lower boundary of a range in meters. The Geo ranking criterion considers all records within the range to be equal.',
          example: 20,
        }),
        value: z.int().exactOptional().openapi({
          description:
...
const BaseSearchParamsSchema = SearchParamsQuerySchema.and(BaseSearchParamsWithoutQuerySchema)
  .readonly()
...
const SearchForHitsSchema = SearchParamsSchema.and(SearchForHitsOptionsSchema)
  .readonly()
  .openapi('SearchForHits')
...
const SearchQuerySchema = z
  .xor([SearchForHitsSchema, SearchForFacetsSchema])
  .readonly()
  .openapi('SearchQuery')
...
const SearchForFacetValuesResponseSchema = z
  .strictObject({
    ...
    exhaustiveFacetsCount: z.boolean().openapi({
      description:
...
const CursorSchema = z
  .strictObject({
    cursor: z.string().exactOptional().openapi({
      description:
...
const BrowseParamsObjectSchema = SearchParamsObjectSchema.and(CursorSchema)
  .readonly()
  .openapi('BrowseParamsObject')
...
const WithPrimarySchema = z
  .strictObject({
    primary: z.string().exactOptional().openapi({
...
const SettingsResponseSchema = IndexSettingsSchema.and(WithPrimarySchema)
  .readonly()
...
const SynonymHitSchema = z
  .strictObject({
    ...
    input: z.string().exactOptional().openapi({
      description:
    ...
    word: z.string().exactOptional().openapi({
      description:
    ...
    placeholder: z.string().exactOptional().openapi({
      description:
...
const ConditionSchema = z
  .strictObject({
    pattern: z.string().exactOptional().openapi({
      description:
    ...
    filters: z.string().exactOptional().openapi({
      description:
...
const EditSchema = z
  .strictObject({
    ...
    insert: z.string().exactOptional().openapi({
...
const AutomaticFacetFilterSchema = z
  .strictObject({
    ...
    disjunctive: z.boolean().default(false).exactOptional().openapi({
      description:
...
const PromoteObjectIDsSchema = z
  .strictObject({
    objectIDs: z.array(ObjectIDSchema).max(100).openapi({
      description:
...
const PromoteSchema = z
  .xor([PromoteObjectIDsSchema, PromoteObjectIDSchema])
  .readonly()
  .openapi('Promote')
...
const ConsequenceSchema = z
  .strictObject({
    ...
    promote: z.array(PromoteSchema).max(300).exactOptional().openapi({
      description:
    ...
    filterPromotes: z.boolean().default(false).exactOptional().openapi({
      description:
    ...
    hide: z
      .array(
        z.strictObject({ objectID: ObjectIDSchema }).openapi({
...
const RuleSchema = z
  .strictObject({
    ...
    conditions: z.array(ConditionSchema).min(0).max(25).exactOptional().openapi({
      description:
...
const DictionaryTypeSchema = z
  .enum(['plurals', 'stopwords', 'compounds'])
  .readonly()
  .openapi('DictionaryType')
...
const ScopeTypeSchema = z.enum(['settings', 'synonyms', 'rules']).readonly().openapi('ScopeType')
...
const FetchedIndexSchema = z
  .strictObject({
    ...
    numberOfPendingTasks: z.int().default(0).openapi({
      description:
        'Number of pending indexing operations. This value is deprecated and should not be used.',
    }),
    pendingTask: z.boolean().default(false).openapi({
      description:
...
const ApiKeyOperationSchema = z
  .enum(['add', 'delete', 'update'])
  .readonly()
  .openapi('ApiKeyOperation')
...
const WatchResponseSchema = z
  .strictObject({
    ...
    data: z.array(z.object({})).exactOptional().openapi({
      description:
    ...
    events: z.array(EventSchema).exactOptional().openapi({
      description: 'in case of error, observability events will be added to the response.',
    }),
    message: z.string().exactOptional().openapi({
      description:
...
const DeletedAtResponse = {
  ...
  content: {
    'application/json': {
      schema: z.strictObject({ taskID: TaskIDSchema, deletedAt: DeletedAtSchema }).openapi({
...
const CreatedAtResponse = {
  ...
  content: {
    'application/json': {
      schema: z.strictObject({ createdAt: CreatedAtSchema }).openapi({
            ...
            .strictObject({
              requests: z.array(
                z
                  .strictObject({
                    action: ActionSchema,
                    body: z.object({}).exactOptional().openapi({
                      description: 'Operation arguments (varies with specified `action`).',
                    }),
                    indexName: IndexNameSchema,
                  })
        ...
        'application/json': {
          schema: z
            .strictObject({
              ...
              anchoring: AnchoringSchema.exactOptional(),
              context: z.string().exactOptional().openapi({
    ...
    body: {
      content: {
        'application/json': {
          schema: z
            .strictObject({
              clearExistingDictionaryEntries: z.boolean().default(false).exactOptional().openapi({
                description:
...
export const get1DictionariesSettingsRoute = createRoute({
  ...
  responses: {
    200: {
      ...
      content: {
        'application/json': {
          schema: z.strictObject({ disableStandardEntries: StandardEntriesSchema }).openapi({
...
export const post1ClustersMappingRoute = createRoute({
  ...
  request: {
    ...
    body: {
      content: {
        'application/json': {
          schema: z.strictObject({ cluster: ClusterNameSchema }).openapi({
    ...
    200: {
      ...
      content: {
        'application/json': {
          schema: z
            .object({
              topUsers: z.array(ClusterNameSchema).openapi({
                description:
                  'Key-value pairs with cluster names as keys and lists of users with the highest number of records per cluster as values.',
              }),
            })
        ...
        'application/json': {
          schema: z
            .strictObject({
              ...
              clusters: z.record(z.string(), z.array(z.string())).exactOptional().openapi({
              ...
              logs: z.array(
                z
                  .object({
                    ...
                    answer_code: z.string().openapi({
                      description: 'HTTP status code of the response.',
                      example: '200',
                    }),
                    query_body: z.string().max(1000).openapi({
                    ...
                    answer: z.string().max(1000).openapi({
                    ...
                    query_headers: z.string().openapi({
                    ...
                    inner_queries: z
                      .array(
                        z
                          .object({
                            index_name: z.string().exactOptional().openapi({
                              description: 'Index targeted by the query.',
                              example: 'products',
                            }),
                            user_token: z.string().exactOptional().openapi({
                              description: 'A user identifier.',
                              example: '93.189.166.128',
                            }),
                            query_id: z.string().exactOptional().openapi({
        ...
        'application/json': {
          schema: z
            .strictObject({
              ...
              scope: z.array(ScopeTypeSchema).exactOptional().openapi({
                description:
...
export const getAccountCopyIndexRoute = createRoute({
  ...
  request: {
    query: z.object({
      sourceIndexName: z.string().openapi({
        param: {
          in: 'query',
      ...
      destinationAppID: z.string().openapi({
        param: {
          in: 'query',
      ...
      destinationApiKey: z.string().openapi({
        param: {
          in: 'query',
      ...
      destinationIndexName: z.string().openapi({
        param: {
          in: 'query',
