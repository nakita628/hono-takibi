...
export async function getStream(options?: ClientRequestOptions) {
  return await parseResponse(client.stream.$get(undefined, options))
}
...
export async function postGraphql(
  ...
  return await parseResponse(client.graphql.$post(args, options))
}
...
export async function postGrpcGateway(
  ...
  return await parseResponse(client['grpc-gateway'].$post(args, options))
}
...
export async function getDeprecatedEndpoint(options?: ClientRequestOptions) {
  return await parseResponse(client['deprecated-endpoint'].$get(undefined, options))
}
