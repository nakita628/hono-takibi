...
const getItemsIndexRouteHandler: RouteHandler<typeof getItemsIndexRoute> = async (c) => {
  return c.json(
    {
      items: Array.from({ length: faker.number.int({ min: 1, max: 5 }) }, () =>
        faker.string.alpha({ length: { min: 5, max: 20 } }),
...
export const api = app
  ...
  .openapi(getApiReverseChibanRoute, getApiReverseChibanRouteHandler)
  .openapi(getPostsIndexRoute, getPostsIndexRouteHandler)
  .openapi(postPostsIndexRoute, postPostsIndexRouteHandler)
  .openapi(getUsersIdIndexRoute, getUsersIdIndexRouteHandler)
  .openapi(getItemsIndexRoute, getItemsIndexRouteHandler)
